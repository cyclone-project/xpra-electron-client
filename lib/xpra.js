...
class Connection {
    ...
    startXpra (conn) {
        ...
        return new Promise((resolve, reject) => {
            ...
            let path = this.xpraPath;
            if (this.platform === 'win32')
                path = '"'+this.xpraPath+'"';

            const script = `${path} attach tcp:localhost:${conn.localPort}`;

            // Start Xpra Connection
            this.xpraChild = Exec(script);
            ...
            this.xpraChild.stderr.on('data', (error) => {
                console.log(error);
            });
    ...
    disconnect () {
        ...
        this.xpraChild.kill('SIGINT');
        this.sshChild.kill('SIGINT');
